...
const ACT_RULES: Record<
  string,
  {
    avoid: Array<"rahu_kalam" | "yamaganda" | "gulika_kalam">;
    preferAbhijit: boolean;
  }
> = {
  travel: { avoid: ["rahu_kalam", "yamaganda"], preferAbhijit: true },
  marriage: {
    avoid: ["rahu_kalam", "yamaganda", "gulika_kalam"],
    preferAbhijit: true,
  },
...
function getLonDeg(res: any): number | undefined {
  ...
  if (res.data && Array.isArray(res.data) && num(res.data[0]) !== undefined)
    return num(res.data[0]);
  if (res.x && Array.isArray(res.x) && num(res.x[0]) !== undefined)
    return num(res.x[0]);
  if (res.position && num(res.position.long) !== undefined)
    return num(res.position.long);
  if (
    res.coordinates &&
    Array.isArray(res.coordinates) &&
    num(res.coordinates[0]) !== undefined
  )
...
function jdToDate(
  ...
  const ms =
    Date.UTC(r.year, r.month - 1, r.day, 0, 0, 0, 0) +
    Math.round((r.hour ?? 0) * 3600 * 1000);
  return new Date(ms);
}
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { ok: false, error: e?.message || "Swiss module missing" },
        { status: 200 }
      );
    }
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            `Swiss ephemeris files missing in "${epheDir}". Add for ${target.getFullYear()}: ${missing.join(
              ", "
    ...
    if (!Number.isFinite(jdUT)) {
      return NextResponse.json(
        { ok: false, error: "Failed to compute julian day (target)." },
        { status: 200 }
      );
    }
    ...
    if (sunRes?.error)
      return NextResponse.json(
        { ok: false, error: `Sun calc: ${sunRes.error}` },
        { status: 200 }
      );
    if (moonRes?.error)
      return NextResponse.json(
        { ok: false, error: `Moon calc: ${moonRes.error}` },
        { status: 200 }
      );
    ...
    if (Number.isFinite(sunLonTrop) && Number.isFinite(moonLonTrop)) {
      ...
      sunRes = await getCalc(
        swe.SE_SUN,
        ephFlag | swe.SEFLG_SIDEREAL | swe.SEFLG_SPEED
      );
      moonRes = await getCalc(
        swe.SE_MOON,
        ephFlag | swe.SEFLG_SIDEREAL | swe.SEFLG_SPEED
      );
      if (sunRes?.error)
        return NextResponse.json(
          { ok: false, error: `Sun calc (sidereal): ${sunRes.error}` },
          { status: 200 }
        );
      if (moonRes?.error)
        return NextResponse.json(
          { ok: false, error: `Moon calc (sidereal): ${moonRes.error}` },
          { status: 200 }
        );
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Swiss result did not include ecliptic longitudes (even after fallback).",
        },
        { status: 200 }
      );
    }
    ...
    const tithi = {
      name: tithiNames[tithiIdx0] ?? `Tithi ${tithiIdx0 + 1}`,
      index: tithiIdx0 + 1,
    };
    ...
    const nakshatra = {
      name: nakNames[nakIdx0] ?? `Nakshatra ${nakIdx0 + 1}`,
      index: nakIdx0 + 1,
    };
    ...
    const { rahu_kalam, yamaganda, gulika_kalam } = computeDayWindows(
      target,
      sunriseLocal,
      sunsetLocal
    );

    const activityKey = (body.activity ?? "general")
      .toLowerCase()
      .replace(/\s+/g, "_");
    ...
    if (birthMoon?.error)
      return NextResponse.json(
        { ok: false, error: `Birth Moon calc: ${birthMoon.error}` },
        { status: 200 }
      );
    const birthMoonLon = getLonDeg(birthMoon);
    if (!Number.isFinite(birthMoonLon))
      return NextResponse.json(
        { ok: false, error: "Birth Moon longitude unavailable" },
        { status: 200 }
      );
    ...
    const nextSafe = (() => {
      ...
      const found =
        safeIntervals.find(([s]) => s >= target) ?? safeIntervals[0];
      const [s, e] = found;
    ...
    return NextResponse.json({
      ...
      verdict: {
        ...
        reasons: [
          tara.isGood
            ? `Tarabala favorable (${tara.name})`
            : tara.isBad
            ? `Tarabala unfavorable (${tara.name})`
            : `Tarabala neutral (${tara.name})`,
          chandra.isBad
            ? `Moon in ${chandra.relation} from Janma rashi (unfavorable)`
            : `Chandra bala OK`,
          inAvoid
            ? `Falls in ${rule.avoid.join(" / ")}`
            : inAbhijit && rule.preferAbhijit
            ? "In Abhijit muhurta"
            : "Outside blocked periods",
        ],
        next_safe_window: nextSafe,
      },
      safe_windows: safeIntervals.map(([s, e]) => ({
        start: s.toISOString(),
        end: e.toISOString(),
      })),
    });
