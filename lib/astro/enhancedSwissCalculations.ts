...
export const ACTIVITY_PLANET_MAP = {
  // Business & Career
  'new_business': { primary: [PLANETS.MERCURY, PLANETS.JUPITER], supportive: [PLANETS.SUN, PLANETS.VENUS], avoid: [PLANETS.SATURN, PLANETS.MARS] },
  'career_change': { primary: [PLANETS.SUN, PLANETS.JUPITER], supportive: [PLANETS.MERCURY], avoid: [PLANETS.SATURN] },
  'financial_investment': { primary: [PLANETS.JUPITER, PLANETS.VENUS], supportive: [PLANETS.MERCURY], avoid: [PLANETS.MARS, PLANETS.SATURN] },
  
  // Relationships & Personal
  'marriage': { primary: [PLANETS.VENUS, PLANETS.JUPITER], supportive: [PLANETS.MOON], avoid: [PLANETS.MARS, PLANETS.SATURN, PLANETS.NORTH_NODE] },
  'proposal': { primary: [PLANETS.VENUS], supportive: [PLANETS.JUPITER, PLANETS.MOON], avoid: [PLANETS.MARS] },
  'relationship_talks': { primary: [PLANETS.VENUS, PLANETS.MERCURY], supportive: [PLANETS.MOON], avoid: [PLANETS.MARS] },
  
  // Health & Wellness  
  'medical_procedure': { primary: [PLANETS.SUN], supportive: [PLANETS.JUPITER], avoid: [PLANETS.MARS, PLANETS.SATURN, PLANETS.NORTH_NODE] },
  'health_treatment': { primary: [PLANETS.SUN, PLANETS.JUPITER], supportive: [PLANETS.MOON], avoid: [PLANETS.MARS, PLANETS.SATURN] },
  'surgery': { primary: [PLANETS.MARS], supportive: [PLANETS.SUN], avoid: [PLANETS.MOON, PLANETS.NORTH_NODE] },
  
  // Education & Communication
  'exam': { primary: [PLANETS.MERCURY, PLANETS.JUPITER], supportive: [PLANETS.SUN], avoid: [PLANETS.SATURN, PLANETS.MARS] },
  'learning': { primary: [PLANETS.MERCURY], supportive: [PLANETS.JUPITER, PLANETS.SUN], avoid: [PLANETS.SATURN] },
  'presentation': { primary: [PLANETS.MERCURY, PLANETS.SUN], supportive: [PLANETS.JUPITER], avoid: [PLANETS.SATURN] },
  
  // Travel & Movement
  'travel': { primary: [PLANETS.MERCURY, PLANETS.JUPITER], supportive: [PLANETS.MOON], avoid: [PLANETS.MARS, PLANETS.SATURN, PLANETS.NORTH_NODE] },
  'relocation': { primary: [PLANETS.MOON, PLANETS.JUPITER], supportive: [PLANETS.MERCURY], avoid: [PLANETS.SATURN, PLANETS.MARS] },
  
  // Spiritual & Personal Growth
  'spiritual_practice': { primary: [PLANETS.JUPITER, PLANETS.NEPTUNE], supportive: [PLANETS.MOON, PLANETS.SUN], avoid: [PLANETS.MARS] },
  'meditation': { primary: [PLANETS.NEPTUNE, PLANETS.MOON], supportive: [PLANETS.JUPITER], avoid: [PLANETS.MARS] },
  
  // General/Default
  'general': { primary: [PLANETS.SUN, PLANETS.MOON], supportive: [PLANETS.JUPITER], avoid: [PLANETS.SATURN, PLANETS.MARS] }
} as const;
...
export interface ActivityAnalysis {
  ...
  planetaryStrengths: Array<{planet: string; strength: number; reason: string}>;
...
export async function calculatePlanetaryPositions(
  ...
  const planetIds = [
    ...
    PLANETS.NORTH_NODE, PLANETS.SOUTH_NODE
  ];
  ...
  for (const planetId of planetIds) {
    try {
      ...
      if (longitude !== undefined && latitude !== undefined) {
        ...
        const planetName = Object.keys(PLANETS)[Object.values(PLANETS).indexOf(planetId)] as keyof typeof PLANETS;
...
export function analyzeActivityTiming(
  ...
  const activityMap = ACTIVITY_PLANET_MAP[activityKey as keyof typeof ACTIVITY_PLANET_MAP] || ACTIVITY_PLANET_MAP.general;
  
  let score = 50; // Start neutral
  const primaryFactors: string[] = [];
  const supportiveFactors: string[] = [];
  const challengingFactors: string[] = [];
  const planetaryStrengths: Array<{planet: string; strength: number; reason: string}> = [];
  ...
  for (const planetId of activityMap.primary) {
    ...
    if (position) {
      ...
      if (strength > 70) {
        score += 20;
        primaryFactors.push(`Strong ${getGlobalPlanetName(planetName)} supports ${activity}`);
      } else if (strength > 50) {
        score += 10;
        supportiveFactors.push(`Favorable ${getGlobalPlanetName(planetName)} position`);
      } else if (strength < 30) {
        score -= 15;
        challengingFactors.push(`Weakened ${getGlobalPlanetName(planetName)} may create obstacles`);
      }
    }
  }
  ...
  for (const planetId of activityMap.supportive) {
    ...
    if (position) {
      ...
      if (strength > 60) {
        score += 8;
        supportiveFactors.push(`Supportive ${getGlobalPlanetName(planetName)} influence`);
      }
    }
  }
  ...
  for (const planetId of activityMap.avoid) {
    ...
    if (position) {
      ...
      if (strength > 60) {
        score -= 12;
        challengingFactors.push(`Strong ${getGlobalPlanetName(planetName)} creates challenging energy`);
      }
    }
  }
  ...
  const globalInterpretation = generateGlobalInterpretation(activity, score, primaryFactors, challengingFactors);
...
// Helper functions
function extractLongitude(result: any): number | undefined {
  if (Array.isArray(result)) return result[0];
  if (typeof result === 'object' && result !== null) {
    return result.longitude || result.lon || result[0];
  }
  return undefined;
}

function extractLatitude(result: any): number | undefined {
  if (Array.isArray(result)) return result[1];
  if (typeof result === 'object' && result !== null) {
    return result.latitude || result.lat || result[1];
  }
  return undefined;
}

function extractSpeed(result: any): number | undefined {
  if (Array.isArray(result)) return result[3];
  if (typeof result === 'object' && result !== null) {
    return result.speed || result.vel || result[3];
  }
  return undefined;
}

function isAspectApplying(p1: PlanetPosition, p2: PlanetPosition, aspectAngle: number): boolean {
...
function getAspectStrength(orb: number, maxOrb: number): 'strong' | 'medium' | 'weak' {
...
function calculatePlanetaryStrength(position: PlanetPosition, aspects: AspectData[]): number {
  let strength = 50; // Base strength
  
  // Add strength based on sign (simplified)
  // This would be enhanced with actual dignity tables
  if (position.sign === 1 || position.sign === 5 || position.sign === 9) { // Fire signs
    strength += 10;
  }
  
  // Retrograde consideration
  if (position.retrograde && position.planet !== 'SUN' && position.planet !== 'MOON') {
    strength -= 10;
  }
  
  // Aspect influences
  const planetAspects = aspects.filter(a => a.planet1 === position.planet || a.planet2 === position.planet);
  for (const aspect of planetAspects) {
    ...
    } else if (aspectData.nature === 'challenging' && aspect.strength !== 'weak') {
...
function getPlanetStrengthReason(position: PlanetPosition, aspects: AspectData[], activity: string): string {
  ...
  const planetAspects = aspects.filter(a => 
    (a.planet1 === position.planet || a.planet2 === position.planet) && a.strength !== 'weak'
  );
  
  const positiveAspects = planetAspects.filter(a => ASPECTS[a.aspect].nature === 'positive');
  const challengingAspects = planetAspects.filter(a => ASPECTS[a.aspect].nature === 'challenging');
  
  if (positiveAspects.length > 0) {
    reasons.push(`favorable aspects from ${positiveAspects.length} planet(s)`);
  }
  if (challengingAspects.length > 0) {
    reasons.push(`challenging aspects from ${challengingAspects.length} planet(s)`);
  }
...
function getGlobalPlanetName(planet: keyof typeof PLANETS): string {
  ...
  return globalNames[planet] || planet;
}

function getAspectInfluence(aspect: AspectData, activity: string): { scoreChange: number; description: string } {
  const aspectData = ASPECTS[aspect.aspect];
  ...
  if (aspectData.nature === 'positive') {
    baseChange = aspect.strength === 'strong' ? 12 : aspect.strength === 'medium' ? 8 : 4;
  } else if (aspectData.nature === 'challenging') {
    baseChange = -(aspect.strength === 'strong' ? 10 : aspect.strength === 'medium' ? 6 : 3);
  }
  
  const description = `${planet1Name}-${planet2Name} ${aspect.aspect.toLowerCase().replace('_', ' ')} ${
...
function getRecommendation(score: number): 'EXCELLENT' | 'GOOD' | 'NEUTRAL' | 'CHALLENGING' | 'AVOID' {
...
function generateGlobalInterpretation(
  ...
  if (recommendation === 'EXCELLENT' || recommendation === 'GOOD') {
    interpretation += ` with strong cosmic support. ${primaryFactors.slice(0, 2).join(' and ')}.`;
  } else if (recommendation === 'CHALLENGING' || recommendation === 'AVOID') {
    interpretation += ` with significant obstacles. Consider postponing as ${challengingFactors.slice(0, 2).join(' and ')}.`;
  } else {
