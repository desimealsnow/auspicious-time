...
const ACTIVITY_TEST_CASES = [
  ...
  },
  {
    ...
    description:
      "Valentine's Day should have mixed but generally positive influences",
  },
  {
    ...
    description:
      "Eclipse periods typically show challenging energy for medical procedures",
  },
] as const;
  ...
  try {
      ...
      for (const [planetName, expectedData] of Object.entries(knownData)) {
        ...
        if (position && expectedData.longitude !== null) {
          ...
          if (diff > expectedData.tolerance) {
            errors.push(
              `${planetName} position off by ${diff.toFixed(2)}° (expected ~${
                expectedData.longitude
              }°, got ${position.longitude.toFixed(2)}°)`
            );
          }
        }
      }
    ...
    if (!sunPosition) {
      ...
      if (sunPosition.speed < 0.8 || sunPosition.speed > 1.2) {
        warnings.push(
          `Sun speed unusual: ${sunPosition.speed.toFixed(
            4
          )}°/day (expected ~0.98)`
        );
      }
    ...
    if (!moonPosition) {
      ...
      if (
        Math.abs(moonPosition.speed) < 10 ||
        Math.abs(moonPosition.speed) > 16
      ) {
        warnings.push(
          `Moon speed unusual: ${moonPosition.speed.toFixed(
            4
          )}°/day (expected 10-15)`
        );
      }
    ...
    for (const position of positions) {
      if (position.longitude < 0 || position.longitude >= 360) {
        errors.push(
          `Invalid longitude for ${position.planet}: ${position.longitude}°`
        );
      }

      if (position.latitude < -90 || position.latitude > 90) {
        errors.push(
          `Invalid latitude for ${position.planet}: ${position.latitude}°`
        );
      }
    }
...
export async function validateActivityRecommendations(
  ...
  try {
    for (const testCase of ACTIVITY_TEST_CASES) {
      ...
      const { analyzeActivityTiming } = await import(
        "./enhancedSwissCalculations"
      );

      const positions = await calculatePlanetaryPositions(swe, jdUT, ephFlag);
      const aspects = calculateAspects(positions);
      const analysis = analyzeActivityTiming(
        testCase.activity,
        positions,
        aspects
      );
      ...
      if (
        analysis.score < testCase.expectedRange.min ||
        analysis.score > testCase.expectedRange.max
      ) {
        warnings.push(
      ...
      if (
        ...
        analysis.supportiveFactors.length === 0 &&
        analysis.challengingFactors.length === 0
      ) {
...
export async function runComprehensiveValidation(
  ...
  const basicTest = await validateSwissEphemeris(
    swe,
    "2024-01-01T12:00:00.000Z",
    ephFlag
  );
  testResults.push({
    test_name: "Basic Planetary Calculations",
    result: basicTest,
  });
  ...
  const solsticeTest = await validateSwissEphemeris(
    swe,
    "2024-06-21T12:00:00.000Z",
    ephFlag
  );
  ...
  testResults.push({
    test_name: "Activity Recommendations",
    result: activityTest,
  });
  ...
  const leapYearTest = await validateSwissEphemeris(
    swe,
  ...
  const averageTime =
    calculationTimes.length > 0
      ...
      : 0;
  const maxTime =
    calculationTimes.length > 0 ? Math.max(...calculationTimes) : 0;
...
export async function quickValidation(
  ...
  try {
    // Use a date within the available ephemeris range (2018 files)
    ...
    if (!Number.isFinite(jdUT)) {
      console.error("Invalid Julian Day calculated");
      return false;
    }

    // Try to calculate just Sun and Moon first
    try {
      const sunResult =
      ...
      if (!sunResult || !moonResult) {
        console.error("Failed to calculate Sun or Moon positions");
        return false;
      }

      // Check if results contain errors
      if (
        (sunResult as { error?: string })?.error ||
        (moonResult as { error?: string })?.error
      ) {
        console.error("Swiss Ephemeris calculation errors:", {
          sun: (sunResult as { error?: string })?.error,
          moon: (moonResult as { error?: string })?.error,
        });
        return false;
      }
      ...
      console.error("Swiss Ephemeris calculation failed:", calcError);
      return false;
    }
  } catch (error) {
