...
function getRAdeg(res: unknown): number | undefined {
  if (!res) return undefined;
  if (typeof res === "object" && res !== null) {
    const obj = res as Record<string, unknown>;
    if (typeof obj.longitude === "number") return obj.longitude; // most builds under EQUATORIAL
    if (typeof obj.ra === "number") return obj.ra; // some builds
  }
...
function getDeclinationDeg(res: unknown): number | undefined {
  if (!res) return undefined;
  if (typeof res === "object" && res !== null) {
    const obj = res as Record<string, unknown>;
    if (typeof obj.latitude === "number") return obj.latitude; // many builds under EQUATORIAL
    if (typeof obj.declination === "number") return obj.declination;
    if (typeof obj.dec === "number") return obj.dec;
  }
...
export function sunAltitudeDeg(
  ...
  const sun: unknown =
    (
      swe as {
        calc_ut_sync?: (jd: number, planet: number, flags: number) => unknown;
      }
    ).calc_ut_sync?.(jd_ut, swe.SE_SUN, flags) ?? // ESM 'sweph' (sync)
    (
      swe as {
        calc_ut?: (jd: number, planet: number, flags: number) => unknown;
      }
    ).calc_ut?.(jd_ut, swe.SE_SUN, flags); // some builds expose sync here

  if (!sun)
    throw new Error(
      "Swiss calc_ut returned no result (callback-only build). Make this function async and use calc_ut_async, or swap to the NOAA path for sun altitude."
    );
  if (sun && typeof sun === "object" && "error" in sun && sun.error) {
    throw new Error(`Sun calc: ${sun.error}`);
  }
  ...
  const sinAlt = clamp(
    Math.sin(phi) * Math.sin(delta) +
      Math.cos(phi) * Math.cos(delta) * Math.cos(Hrad),
    -1,
    1
  );
...
export function sunriseSunsetAround(
  ...
  let sunriseLo: number | null = null,
    sunriseHi: number | null = null;
  let sunsetLo: number | null = null,
    sunsetHi: number | null = null;
  ...
  for (let jd = prevJD; jd <= jdCenter + spanDays / 2 + 1e-9; jd += stepDays) {
    ...
    if (!Number.isNaN(prevAlt)) {
      if (prevAlt < 0 && alt >= 0 && sunriseLo === null) {
        sunriseLo = prevJD;
        sunriseHi = jd;
      }
      if (prevAlt > 0 && alt <= 0 && sunsetLo === null) {
        sunsetLo = prevJD;
        sunsetHi = jd;
      }
    }
  ...
  if (sunriseLo !== null && sunriseHi !== null)
    out.sunriseJD = refine(sunriseLo, sunriseHi);
  if (sunsetLo !== null && sunsetHi !== null)
    out.sunsetJD = refine(sunsetLo, sunsetHi);
  return out;
}
